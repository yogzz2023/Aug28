    ...
    for group_idx, group in enumerate(measurement_groups):
        ...
        for i, (rng, azm, ele, mt) in enumerate(group):
            ...
            if not kalman_filter.first_rep_flag:
                ...
                predicted_states = [kalman_filter.Sf.flatten()[:3]]  # Assuming single state prediction for simplicity
                measurements = np.array([sph2cart(azm, ele, rng) for rng, azm, ele, _ in group])
                cost_matrix = np.linalg.norm(predicted_states - measurements[:, np.newaxis], axis=2)

                # Apply Munkres algorithm
                row_ind, col_ind = munkres_algorithm(cost_matrix)

                # Update with matched measurements
                for match in zip(row_ind, col_ind):
                    Z = np.array([[measurements[match[1], 0]], [measurements[match[1], 1]], [measurements[match[1], 2]]])
