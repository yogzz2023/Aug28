...
# Store track and report information for plotting
track_report_info = []  # List to store (time, track_id, track_position, report_position)
...
def main():
    ...
    for group_idx, group in enumerate(measurement_groups):
        ...
        for i, (rng, azm, ele, mt) in enumerate(group):
            ...
            if not kalman_filter.first_rep_flag:
                ...
                # Store track to report association for plotting
                track_id = f"Track-{group_idx+1}"
                track_position = kalman_filter.Sf.flatten()[:3].tolist()
                report_position = Z.flatten().tolist()
                track_report_info.append((mt, track_id, track_position, report_position))
    ...
    # Plot track and report positions
    plt.figure(figsize=(12, 6))
    plt.subplot(facecolor="white")

    for mt, track_id, track_position, report_position in track_report_info:
        plt.scatter(mt, track_position[0], color='green', marker='*', label=f'Track: {track_id}')
        plt.scatter(mt, report_position[0], color='red', marker='o', label=f'Report: {track_id}')
        plt.text(mt, track_position[0], f'Track Pos: {track_position}', fontsize=8, color='green')
        plt.text(mt, report_position[0], f'Report Pos: {report_position}', fontsize=8, color='red')

    plt.xlabel('Time', color='black')
    plt.ylabel('Position (x)', color='black')
    plt.title('Track and Report Positions Over Time', color='black')
